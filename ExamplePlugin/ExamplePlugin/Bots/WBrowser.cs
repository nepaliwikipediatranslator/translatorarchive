...
//calculator
        ...
        {
            MessageBox.Show("Wikipedia Translator");
        }
       ...
       private void editToolStripMenuItem1_Click(object sender, EventArgs e)
       {
        ...
        {
            if (this.adrBarTextBox.Text.ToLower().Contains("wikipedia"))
            {
                try
                {
                    var address = adrBarTextBox.Text.ToLower().IndexOf("wiki");

                    //MessageBox.Show(address.ToString());

                    var startindex = adrBarTextBox.Text.ToLower().IndexOf("wikipedia.org/wiki/") + ("wikipedia.org/wiki/").Length;

                    var length = adrBarTextBox.Text.Length - startindex;
                    var articleName = adrBarTextBox.Text.Substring(startindex, length);
                    this.Hide();
                    mainWindow.LoadWikiArticle(articleName);
